...
from top_paper_mcp_server.config import Settings
...
def test_storage_path_default(mock_resolve, mock_mkdir):
    ...
    mock_resolve.side_effect = lambda: Path.home() / ".top-paper-mcp-server" / "papers"

    settings = Settings()
    expected_path = Path.home() / ".top-paper-mcp-server" / "papers"
