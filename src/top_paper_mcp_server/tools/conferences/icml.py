"""ICML paper source via official conference website."""
...
class ICMLSource(ConferenceSource):
    """ICML paper source via official website."""
    ...
    async def search(
        ...
        """Search papers in ICML conference."""
        url = f"{ICML_BASE_URL}/virtual/{year}/papers.html"
        ...
        try:
            ...
            logger.error(f"ICML search error: {e}")
            return []
        except Exception as e:
            logger.exception(f"ICML search failed: {e}")
            return []

    def _parse_paper_list(
        self, html: str, year: int, query: str, max_results: int
    ) -> List[PaperMetadata]:
        """Parse paper list from ICML virtual page."""
        ...
        for paper_div in soup.find_all("div", class_="paper"):
            ...
            pdf_url = f"{ICML_BASE_URL}/virtual/{year}/{paper_id}.pdf"
            detail_url = f"{ICML_BASE_URL}/virtual/{year}/{paper_id}"
    ...
    async def get_paper(
        ...
        for year in range(2020, 2030):
            url = f"{ICML_BASE_URL}/virtual/{year}/{paper_id}"
            try:
    ...
    async def download_paper(self, paper_id: str, conference: str) -> Dict[str, Any]:
        ...
        return {"status": "error", "message": f"Paper {paper_id} not found in ICML"}
