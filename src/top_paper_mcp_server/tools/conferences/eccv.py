"""ECCV paper source via ECVA (European Computer Vision Association)."""
...
class ECCVSource(ConferenceSource):
    """ECCV paper source via ECVA website."""
    ...
    async def search(
        ...
        """Search papers in ECCV conference."""
        url = f"{ECVA_BASE_URL}/virtual/{year}/papers.html"
        ...
        try:
            ...
            logger.error(f"ECCV search error: {e}")
            fallback_url = f"https://eccv.ecva.net/virtual/{year}/papers.html"
            try:
                async with httpx.AsyncClient(
                    ...
                    return self._parse_paper_list(
                        response.text, year, query, max_results
                    )
            except Exception:
                return []
        except Exception as e:
            logger.exception(f"ECCV search failed: {e}")
            return []

    def _parse_paper_list(
        self, html: str, year: int, query: str, max_results: int
    ) -> List[PaperMetadata]:
        """Parse paper list from ECCV virtual page."""
        ...
        for paper_div in soup.find_all("div", class_="paper"):
            ...
            pdf_url = f"{ECVA_BASE_URL}/virtual/{year}/{paper_id}.pdf"
            detail_url = f"{ECVA_BASE_URL}/virtual/{year}/{paper_id}"
    ...
    async def get_paper(
        ...
        for year in range(2018, 2030):
            url = f"{ECVA_BASE_URL}/virtual/{year}/{paper_id}"
            try:
    ...
    async def download_paper(self, paper_id: str, conference: str) -> Dict[str, Any]:
        ...
        return {"status": "error", "message": f"Paper {paper_id} not found in ECCV"}
